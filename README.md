...
`catui` alone and stays version-locked to the same glyphs and flavors.
...
## Editing catui while an app consumes it
...
dependency_overrides:
  catui:
    path: ../clockodile/packages/catui
```
...
then reaches into `path:`) and catui's history is tangled with an app's. Both
...
`packages/catui` is already self-contained with its own pubspec, tests and
